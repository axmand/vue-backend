...
define(['baseControllers', 'simplecolor', 'jquery'], function (baseControllers, simplecolor, $) {
    baseControllers.controller('topicCreateController', function (
        ...
        $scope.$on('$ionicView.enter', function () {
            ...
            //当前编辑的专题
            $scope.nowTopic = $scope.model == 1 ? {} : $$dataCache.getData('topicDetail');
            //当前编辑的详情页
        ...
        $scope.topicPost = function () {
            ...
            $$dataBase.postData('BackendCommercialCreateTopicPost', postContent).then(function (data) {
                if (data.status == 'ok') {
                    $$toast('创建成功', 'success');
                    $window.history.back();
                } else {
