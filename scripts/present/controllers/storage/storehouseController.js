/**
*   仓库类
*   @author yellow date 2016/8/22
*/
define(['baseControllers'], function (baseControllers) {

    baseControllers.controller('storehouseController', function ($filter, $$dataBase, $$bootstrapModal, $scope, groupService, $ionicSideMenuDelegate, $$tree, $$toast) {

        var config = $$dataBase.getAppConfig();

        var selectType = null, selectNode = null, tree, selectKey = "CommercialWhareHouse";
        //设定分类
        selectType = config.ECategory[selectKey];

        var _refresh = function () {
            $$dataBase.getCategory(config.ECategory[selectKey]).then(function (data) {
                tree = {
        ...
        $scope.$on("$ionicView.enter", function () {
            if (arguments[0].targetScope == arguments[0].currentScope) {
                //进入时初始化树形绘制容器
                $$tree.setContainerId('tree-container-storeHouse', selectNodeFn, changeFatherFn);
                _refresh();
            }
        });

        $$bootstrapModal.fromTemplateUrl('scripts/present/views/modal/mumCategoryStoreHouse.html', {
            scope: $scope
        }).then(function (modal) {
            categoryModal = modal;
        });
